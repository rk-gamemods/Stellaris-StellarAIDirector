...
NSC3_WORKSHOP_ROOT = Path(r"C:\Steam\steamapps\workshop\content\281990\683230077")
...
IDENTITY_FLEET_COMPOSITION_PATH = (
    ...
    / "common"
    / "ship_sizes"
    / "zzzz_staid_17_nsc3_identity_fleet_composition_ship_sizes.txt"
)
IDENTITY_FLEET_COMPOSITION_SOURCES = {
    "00_ship_sizes.txt": {
        "corvette": "38a75371373240966e3b0512e81ea7fa8a313a4b3776ceb93abac97fff3c3aa9",
        "frigate": "acced2c5ca389e042dd474f036bb8bad04d959a2eb941b829bd30277f0d9b39c",
        "destroyer": "c8f8046548062993627ede0a85bdb605c8e4c3a80b7096f04a334dc87db358e1",
        "cruiser": "4415d62b444d7b934f4686c2cc01ddb54f304e5e520a7188501b58b7a7f990bf",
        "battleship": "696d6f8177e1fd1e75d8b73f2b5dcae27d00d2eafb9408217a5ff704fe4f4e24",
    },
    "nsc_ship_sizes.txt": {
        "StrikeCruiser": "9b8fae043ce1afdfaa5eaf5603d9dae99ba8f99455f85d4735f77414338f045a",
        "Battlecruiser": "73f151ba30ffe1ce1baba616d3b5e5245d2e7ba5b601ba73c83dd4e09b244b92",
        "Carrier": "e8420148ad3039fa154be42291b7801f6dcb3be3a39852bb3e7fe6a639a7c2d1",
...
def identity_fleet_composition_modifiers(object_id: str) -> list[str]:
    """Stop requesting sub-Battleship hulls after Battleships unlock."""
    ...
    if object_id not in {
        ...
        return []
    return [
        "\t\t\tmodifier = { factor = 0 has_technology = tech_battleships }",
    ]


def identity_fleet_composition_object_text(
    source_text: str,
    object_id: str,
    expected_sha256: str,
) -> str:
    """Copy one hash-locked NSC3 hull and change only its ai_ship_data."""
    ...
    if actual_sha256 != expected_sha256:
        raise ValueError(
            f"Active NSC3 ship size drifted: {object_id} "
            f"expected={expected_sha256} actual={actual_sha256}"
        )
    modifiers = identity_fleet_composition_modifiers(object_id)
    if not modifiers:
        return block
    ai_ship_data = extract_top_level_object_text(block, "ai_ship_data")
    modified_ai_ship_data = append_child_block_clause(
        ai_ship_data,
        "fraction",
        "\n".join(modifiers),
    )
    if block.count(ai_ship_data) != 1:
        raise ValueError(f"NSC3 {object_id} does not have one replaceable ai_ship_data block")
    return block.replace(ai_ship_data, modified_ai_ship_data, 1)


def render_identity_fleet_composition_artifact() -> str:
    """Render the exact nine-object NSC3 fleet-composition compatibility file."""

    lines = [
        "# Generated by tools/generate_stellar_ai_identity_fleet_composition.py.",
        "# Hash-locked full-object copies of the active NSC3 final winners.",
        "# Only ai_ship_data late-tech fractions change; hull graph and early behavior remain parent-owned.",
        "# This influences desired composition but cannot force or refresh serialized fleet templates.",
        "",
    ]
    source_rows = [
        {
    ...
    variables = route_override_file_variables(source_rows)
    if variables:
        lines.extend(variables)
        lines.append("")
    object_count = 0
    for source_file, object_hashes in IDENTITY_FLEET_COMPOSITION_SOURCES.items():
        source_path = NSC3_WORKSHOP_ROOT / "common" / "ship_sizes" / source_file
        source_text = read_text(source_path)
        for object_id, expected_sha256 in object_hashes.items():
            lines.append(f"# source = NSC3/common/ship_sizes/{source_file}")
            lines.append(f"# source_object_sha256 = {expected_sha256}")
            lines.append(
            ...
            object_count += 1
    if object_count != 9:
        raise ValueError("Identity fleet-composition manifest must remain exactly nine objects")
    rendered = "\n".join(lines).rstrip() + "\n"
    parse_pdx(rendered)
    return rendered
