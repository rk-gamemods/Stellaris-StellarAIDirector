...
\t\t}""",
    ...
    research = append_child_block_clause(
        ...
        """\t\tOR = {
\t\t\towner = { is_ai = no }
\t\t\thas_carrier_flag = staid_research_plan_claimed
\t\t}""",
    )
    research = append_child_block_clause(
        research,
        "weight_modifier",
        """\t\t# A plan claim is a designation commitment, not merely eligibility.
\t\tmodifier = {
\t\t\tadd = 100000
\t\t\thas_carrier_flag = staid_research_plan_claimed
\t\t}""",
...
\t# A Tech-World must construct the zone that makes research labs eligible.
\tadditional_ai_weight = {
\t\tmodifier = {
\t\t\tadd = 100000
...
# this connects the existing Tech-World designation commitment to that prerequisite.
...
def generate_mod_files(rows: list[dict[str, Any]] | None = None) -> None:
    ...
    for stale_path in (
        ...
        # Remove the obsolete cloak-disable workaround. Science ships must
        # retain access to vanilla cloaking components.
...
def research_colony_plan_triggers_text() -> str:
    return '''# Colony/carrier-scope build-out plan selectors. The first research role
...
\t\t\t\thas_carrier_flag = staid_research_plan_claimed
\t\t\t}}
\t\t\tremove_carrier_flag = staid_research_plan_claimed
\t\t}}
