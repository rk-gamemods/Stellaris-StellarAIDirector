...
| M04 | Separate base, triggered, optimistic, conservative, and prerequisite-qualified benefits instead of silently mixing conditional benefits. | Jobs/buildings/development/plan CSVs; readiness scenarios | Jobs, buildings, development rows, strategic infrastructure rows, and plan rows now preserve base/triggered/optimistic resource columns where applicable. Buildings/development also carry conservative scenario columns, with conservative output equal to base output and conservative upkeep equal to optimistic upkeep. Prerequisite-qualified scenarios remain part of M11/M13 readiness work. | partial | Focused scenario unit test passes; JData scenario arithmetic checks show 0 mismatch rows for 826 buildings and 547 development rows; JData indexes validate. | Create prerequisite-qualified readiness scenarios that consume source gates, capital tiers, and fallback-before-unlock candidates. |
...
1. Add explicit M02 schema tests for the existing source/category/job/output/
   ...
   guarded.
2. Create prerequisite-qualified readiness scenarios that consume source gates,
   capital tiers, fallback-before-unlock candidates, and automation replacement
   limits.
3. Add explicit benefit-class taxonomy/status columns, starting with the
   benefit classes already partially detected in strategic infrastructure rows.
4. Expand role-family rows for capital, habitat, ring-world, arcology,
