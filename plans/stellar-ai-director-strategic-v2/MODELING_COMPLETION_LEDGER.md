...
| M16 | Add deterministic completion validation: schema, row counts, unresolved flags, required columns, and known rows. | `tools/tests/test_stellar_ai_director.py`; optional dedicated test file | Added focused tests for active-winning-building reconciliation, promoted Gigas resource columns/resource coverage, source gate columns, scenario resource columns/arithmetic, build-plan readiness rows, strategic benefit taxonomy coverage, and expanded role-family coverage. | partial | Focused tests and full `python -m unittest discover -s tools\tests` pass. | Add tests for known `transit_hub`, unresolved blocker accounting, and full schema/row-count contracts. |
| M17 | Regenerate from a clean command and index/validate every refreshed CSV with JDataMunch. | Generator outputs; JData datasets | Regenerated all research-capacity CSVs after custom resource, coverage-table, source-gate, scenario-column, readiness, benefit-taxonomy, and expanded-role changes; role rows expanded intentionally. | partial | `python tools\build_stellar_ai_research_capacity_dataset.py`; JData `index_local` and `validate_index` ok for ten modeling datasets. | Repeat after each later artifact change; final pass must cover every new/refreshed CSV. |
...
1. Add explicit M02 schema tests for the existing source/category/job/output/
   ...
   guarded.
2. Review benefit taxonomy detected-unvalued rows and define class-specific
   formulas or detected-only consumer policy.
3. Review readiness fallback quality by role and define the build-plan consumer
   rules for temporary/permanent replacement behavior.
4. Resolve unknown jobs, unresolved variables, and blocker accounting into
   generated blocker rows or source-backed exclusions.
